...
export default function LoginForm({ urlPath }: { urlPath?: string }) {
    const appUrl = import.meta.env.VITE_BASEURL;
            ...
            {registrationSuccess
                ...
                : <Paper>
                    <Box onSubmit={onSubmit} component='form'
                        ...
                        <Box sx={{ marginTop: 4, display: "flex", justifyContent: 'center', flexDirection: 'column', gap: 1 }}>
                            <Button fullWidth size="large" type="submit" loading={isLogingIn} color="success" variant="contained">Login</Button>
                            <Button
                                fullWidth size="large"
                                startIcon={<GitHub />}
                                variant="contained"
                                sx={{ background: 'black' }}
                                component="a" href={`${appUrl}externalauth/signin`} target="_blank">
                                Login with github
                            </Button>
                        </Box>
                    </Box>
